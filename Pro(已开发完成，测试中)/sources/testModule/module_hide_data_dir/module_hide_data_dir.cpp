...
#define JSON_FILE_NAME "hide_dir.json"
#define MOD_VER "0.0.1"

static std::string get_json_file_path(const char* module_private_dir) {
    return std::string(module_private_dir) + JSON_FILE_NAME;
}
...
int skroot_module_main(const char* root_key, const char* module_private_dir) {
    ...
    //读取配置文件内容
    std::string file_path = get_json_file_path(module_private_dir);
    std::string json;
    read_text_file(file_path.c_str(), json);
...
public:
    void onPrepareCreate(const char* root_key, const char* module_private_dir, uint32_t port) override {
        m_root_key = root_key;
        m_json_file_path = get_json_file_path(module_private_dir);
    }
    ...
    bool handlePost(CivetServer* server, struct mg_connection* conn) override {
        ...
        else if(path == "/getHiddenDirsJson") read_text_file(m_json_file_path.c_str(), resp);
        else if(path == "/setHiddenDirsJson") resp = write_text_file(m_json_file_path.c_str(), body) ? "OK" : "FAILED";
        mg_printf(conn,
                  "HTTP/1.1 200 OK\r\n"
